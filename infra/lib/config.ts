...
export const loadDeploySettings = (scope: Construct): DeploySettings => {
  ...
  return {
    ...
    runtimeEnv: {
      ...
      IMAGE_PROVIDER: ctx(scope, 'imageProvider') ?? 'bedrock',
      OPENAI_IMAGE_MODEL_ID: ctx(scope, 'openAiImageModelId') ?? '',
      OPENAI_IMAGE_SIZE: ctx(scope, 'openAiImageSize') ?? '1152x2048',
      OPENAI_IMAGE_QUALITY: ctx(scope, 'openAiImageQuality') ?? 'high',
